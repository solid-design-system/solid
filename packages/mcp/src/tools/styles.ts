...
export const stylesTool = (server: McpServer) => {
  server.registerTool(
    'styles',
    {
      ...
      inputSchema: {
        ...
        doc: z
          ...
          .optional()
          .describe(
            'Package-level guide slug, e.g. "installation", "usage". ' +
    ...
    async ({ style, example, doc }) => {
            ...
            content: [
              {
                type: 'text',
                text:
                  `No package doc found for "${doc}". Available docs:\n` +
      ...
      // --- no args: index ---
      const [styles, packageDocs] = await Promise.all([
        getAvailableStyles(),
        getAvailablePackageDocs()
      ]);
