...
- Always use proper HTML heading tags (e.g., <h1>, <h2>) for logical structure in the document, even if style classes decouple font size from H-tags, to ensure consistent design, accessibility, and flexibility across use cases. H-tags are applied in the CMS following semantic headline order.
...
<OverviewFormatter story={HeadlineStories.Default}>{content}</OverviewFormatter>
