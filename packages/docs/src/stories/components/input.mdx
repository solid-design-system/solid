...
- Handle locale-aware number input with automatic formatting (formatted-number type).
...
Use the \`formatted-number\` type for locale-aware number input when you need:

- Automatic number formatting based on the user's locale (or a specified locale).
- Input that handles different decimal and grouping separators (e.g., 1,234.56 in English vs. 1.234,56 in German).
- Stepping and validation with min/max/step constraints.
- Form submission with raw numeric values regardless of display format.
