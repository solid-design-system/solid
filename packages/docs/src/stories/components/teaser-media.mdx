...
- Adopt a consistent, semantically correct heading hierarchy to enable quick scanning across multiple teasers on a page. Teaser headings should be the first item in the teaser's DOM structure, introducing a new thematic region and separating the following content from the previous region.
- Include descriptive alt text for the media slot only if the image adds relevant new information in the context of the teaser. The image description must not repeat the teaser title. If the image is purely decorative, use empty alt text to ensure it’s ignored by assistive technologies.
...
- If the teaser contains interactive elements, ensure they replicate the same behavior on focus as on hover (increasing background opacity).
