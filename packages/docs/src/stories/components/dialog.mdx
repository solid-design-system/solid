...
import * as SdDialogStories from './dialog.stories';

export const links = {
  'storybook-docs': './?path=/docs/components-sd-dialog--docs',
  'figma-library':
...
Used over other content. It requires an interaction from the user before they can return to what is underneath.

- Use the "headline" slot to add a headline. Since the wrapper of the slot is an <code>&lt;h2&gt;</code> by default, the slot should be a generic element (<code>&lt;span&gt;, &lt;div&gt;</code>).
- Use the "default" slot to add main content. "Default" slot is always scrollable.
- Use the "footer" slot to add action elements. "Footer" slot is always fixed.
...
<sd-notification variant="warning" open>**Known browser issues:** When the headline is set by slot instead of attribute, the dialog name is not being announced by VoiceOver in Chrome and Firefox.</sd-notification>`;

<Meta title="Components/sd-dialog/Overview" />

<OverviewFormatter story={SdDialogStories.Default}>{content}</OverviewFormatter>
