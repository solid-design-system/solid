    ...
    return html`
      ${['default', 'play-icon', 'pause-icon'].map(slot => {
        ...
        if (slot === 'default') {
          ...
          value = `<sd-icon class="slot slot--border slot--background text-white text-3xl" slot="play-icon" library="internal" name="start"></sd-icon>`;
        } else if (slot === 'pause-icon') {
          value = `<sd-icon class="slot slot--border slot--background text-white text-3xl" slot="pause-icon" library="internal" name="pause"></sd-icon>`;
        }
