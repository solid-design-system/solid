...
> **Deprecated** — This component is deprecated. Use the [\`sd-skeleton\` CSS style](./?path=/docs/styles-sd-skeleton--docs) instead. Apply the \`sd-skeleton\` class directly to your content element (e.g. \`<p class="sd-skeleton">text</p>\`) and remove it once loading is complete.
