...
test('Docs', async ({ page }) => {
  await page.goto('http://127.0.0.1:6998/iframe.html?globals=&args=&id=templates-dialog--docs&viewMode=story');
  await expect(page.locator('body')).toMatchAriaSnapshot(`
    ...
    - complementary:
      - navigation "Table of contents":
        ...
        - list:
          - listitem:
            - link "Dialog with Button Group":
              - /url: "#dialog-with-button-group"
          - listitem:
            - link "Dialog with scrollable content":
              - /url: "#dialog-with-scrollable-content"
          - listitem:
            - link "Prevent Closing":
              - /url: "#prevent-closing"
    - heading "Dialog" [level=1]
    - heading "Dialog with Button Group" [level=3]
    - button "Open Dialog"
    ...
    - heading "Dialog with scrollable content" [level=3]
    - button "Open Dialog"
    - dialog "Terms of use":
      - banner:
    ...
    - button "Show code"
    - button "Edit on CodePen"
    - heading "Prevent Closing" [level=3]
    - paragraph:
      - text: This shows ways to prevent closing
      - code: sd-dialog
      - text: . This is useful for instances when data loss will occur.
    - paragraph:
      - text: To keep the dialog open in such cases, cancel the
      - code: sd-request-close
      - text: event. When canceled, the dialog will remain open and pulse briefly to draw the user’s attention to it. Use
      - code: event.detail.source
      - text: to specify a closing trigger.
    - button "Open Dialog"
