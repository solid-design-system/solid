...
Solid Components come without any theme by default. You have to install `@solid-design-system/tokens` and integrate a theme via CSS yourself:

> This only has to be done once per document, every application and component using Solid Components on that page can then automatically use the included themes.

```bash
npm install @solid-design-system/tokens
```

Then include the CSS file of the theme you want to use. For example, to use `ui-light` theme, add the following line to your HTML head:
...
For more information about themes, please refer to the documentation in the [tokens package](?path=/docs/packages-tokens-installation--docs).
