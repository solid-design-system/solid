...
The v7 release removes deprecated CSS variables, adds built-in theme fallbacks defaulting to `ui-light`, adds an optional import to use the legacy variables fallback and introduces new tokens for the consolidated `sd-chip` component (which replaces `sd-flag`).
