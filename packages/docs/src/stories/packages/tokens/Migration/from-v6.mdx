...
The v6 release removes deprecated CSS variables, adds built-in theme fallbacks defaulting to `ui-light`, adds an optional import to use the legacy variables fallback and introduces new tokens for the consolidated `sd-chip` component (which replaces `sd-flag`).
...
## Consolidation of `sd-chip` and `sd-flag`

**Removed CSS variables**

Please refer to the "Deprecated CSS variables (tokens)" section to see the removed css variables. Update any theme or override it with the new variables.
