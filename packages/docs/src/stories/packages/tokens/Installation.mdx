...
CSS variables used for custom theming can be phased out as versions are updated.

- All deprecated variables are clearly marked in each component's documentation
- All deprecated variables are clearly marked and recommended replacements provided in the <sd-link href="https://solid-design-system.fe.union-investment.de/docs/?path=/docs/packages-tokens-installation--docs#css-variables-deprecation">Usage</sd-link> page under "Deprecated CSS variables (tokens)"

If you manage an environment that still requires the use of these variables, you can use the `legacy-variables.css` stylesheet
as fallback layer. Make sure to import the stylesheet before any other stylesheet so it overrides other configurations:
...
That's it — you're ready to build. Head over to the <sd-link href="?path=/docs/packages-tokens-usage--docs">Usage page</sd-link> to learn about local development, embedded app compatibility, and versioning coordination.
