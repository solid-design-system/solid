...
If you import any SDS theme stylesheet from this package (for example `ui-light.css`), no extra action is required:
fallbacks are included automatically.

The same applies when importing Tailwind from this package (`@solid-design-system/tokens/dist/themes/tailwind.css`).

Use `token-fallbacks.css` only when you want a standalone compatibility layer without importing an SDS theme stylesheet.
