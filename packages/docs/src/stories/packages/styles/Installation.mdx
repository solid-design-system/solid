...
Solid Styles come without any theme by default. You have to install `@solid-design-system/tokens` and integrate a theme via CSS yourself:

> This only has to be done once per document, every application and component using Solid Styles on that page can then automatically use the included themes.
