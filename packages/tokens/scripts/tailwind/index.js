...
// ---------------------------------------------------------------------------
// Tailwind v3 JSON config generator
//
// Generates a `tokens.tailwind.json` file containing a complete Tailwind v3
// `theme.extend`-compatible configuration derived from the compiled theme tokens.
// ---------------------------------------------------------------------------
 ...
 * @param {object} uiCore - The "UI Core" section from tokens.json
 * @returns {object}
 */
function generateTailwindConfig(themejs, uiCore = {}) {
  const { theme: themeTokens = [], utilities = [] } = themejs;
  const rawSpacing = flattenRawTokens(uiCore, ['spacing']);
  const rawLineHeight = flattenRawTokens(uiCore, ['lineHeights']);
 ...
 */
export function generateTailwindJson(themejs, uiCore = {}) {
  const config = generateTailwindConfig(themejs, uiCore);
