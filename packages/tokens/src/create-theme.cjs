...
const resolveValue = (value) => {
	...
	if (value.startsWith("{")) {
		...
		const resolvedValue =
			resolvePath(path, "UI Core") || resolvePath(path, "UI Semantic");
...
const getOpacities = () => {
	...
	Object.entries(tokens["UI Semantic"].opacity)
		...
		.forEach(({ name, value, description }) => {
			...
			result[sanitizeValue(name)] = `var(--sd-opacity-${sanitizeValue(
				name
			)}, ${convertedValue})${
...
const getBorderRadius = () => {
	...
	Object.entries(tokens["UI Semantic"].rounded)
		...
		.forEach(({ name, value, description }) => {
			result[
				sanitizeValue(name)
			] = `var(--sd-border-radius-${name}, ${resolveValue(value)})${
				description ? ` /* ${description} */` : ""
			}`;
		});
	return result;
...
const getSpacings = () => {
	...
	Object.entries({
		...
		.map(([name, value]) => ({ name, ...value }))
		.concat(
			Object.entries(tokens["UI Semantic"].sizing).map(
...
const getCoreTokensByType = (tokenType, cssVariableScope) => {
	...
	keys.filter((key) => {
		...
		if (type === tokenType) {
			...
			result[
				keyWithoutUtility
			] = `var(--sd-${cssVariableScope}-${sanitizeKey(
				keyWithoutUtility
			)}, ${obj[key]["value"]})${
				description ? ` /* ${description} */` : ""
			}`;
