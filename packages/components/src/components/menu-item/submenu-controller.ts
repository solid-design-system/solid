...
export class SubmenuController implements ReactiveController {
  ...
  constructor(host: ReactiveControllerHost & SdMenuItem, hasSlotController: HasSlotController) {
  ...
  private addListeners() {
    ...
    if (!this.isPopupConnected) {
      if (this.popupRef.value) {
        ...
        this.popupRef.value.addEventListener('sd-reposition', this.handlePopupReposition);
  ...
  private removeListeners() {
    ...
    if (this.isPopupConnected) {
      if (this.popupRef.value) {
        ...
        this.popupRef.value.removeEventListener('sd-reposition', this.handlePopupReposition);
  ...
  private handleSubmenuEntry(event: KeyboardEvent) {
    ...
    for (const elt of submenuSlot.assignedElements()) {
      menuItems = elt.querySelectorAll("sd-menu-item, [role^='menuitem']");
  ...
  private handlePopupReposition = () => {
    const submenuSlot: HTMLSlotElement | null = this.host.renderRoot.querySelector("slot[name='submenu']");
    const menu = submenuSlot?.assignedElements({ flatten: true }).filter(el => el.localName === 'sd-menu')[0];
  ...
  renderSubmenu() {
    ...
    // Always render the slot, but conditionally render the outer <sd-popup>
    if (!this.isConnected) {
