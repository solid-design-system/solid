...
export default class SdOption extends SolidElement {
  ...
  @state() protected current = false; // the user has keyed into the option, but hasn't selected it yet (shows a highlight)
  @state() protected selected = false; // the option is selected and has aria-selected="true"
