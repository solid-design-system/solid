 ...
 * @cssproperty --sd-form-control-color-border - The default border color for the datepicker.
 ...
 * @cssproperty --sd-form-control--invalid-color-background - The background color for the input when it is invalid.
 * @cssproperty --sd-form-control__listbox-border-bottom-left-radius - The border radius for the bottom left corner of the datepicker.
 ...
 * @cssproperty --sd-datepicker__date-item--default-color-border - The default border color for date items.
 * @cssproperty --sd-datepicker__date-item--selected-color-text - The text color for selected date items.
 * @cssproperty --sd-datepicker__date-item--hover--prev-next-color-background - The background color for date items in hover state when they are previous/next month days.
 * @cssproperty --sd-datepicker__date-item--hover--prev-next-color-text - The text color for date items in hover state when they are previous/next month days.
 * @cssproperty --sd-datepicker__date-item--current-font-weight - The font weight for the current date item.
 * @cssproperty --sd-datepicker__date-item--selected-color-background - The background color for selected date items.
...
export default class SdDatepicker extends SolidElement implements SolidFormControl {
  ...
  /** Suppresses focus-opens-the-calendar while focus is being handed back to the input on close. */
  private restoringFocusAfterClose = false;
  ...
  private closeAndRestoreFocus() {
    ...
    this.restoringFocusAfterClose = true;

    this.updateComplete.then(() => {
      this.input?.focus({ preventScroll: true });
      this.restoringFocusAfterClose = false;
    });
  }
  ...
  private handleFocus() {
    ...
    if (!this.open && !this.restoringFocusAfterClose && !this.disabled && !this.visuallyDisabled && !this.readonly) {
      this.show();
    }
    this.emit('sd-focus');
  }
  ...
  private focusInitialGridDay() {
    ...
    if (this.range) {
      ...
      if (inViewAndEnabled(selected)) target = DateUtils.startOfDayLocal(selected!);
    }
  ...
  private onKeyDown = (ev: KeyboardEvent) => {
    ...
    if (ev.key === 'Tab') {
      ...
      this.shadowRoot?.querySelector<HTMLButtonElement>('button[part="prev-year-button"]')?.focus({
        preventScroll: true
      });
      return;
    }
        ...
        <div class="flex flex-row items-center w-full justify-between mb-3" part="header">
          <div class="flex items-center">
            ...
            <button
              ...
              tabindex="0"
              class=${cx(
                'nav prev relative w-6 h-6 hover:cursor-pointer focus-visible:focus-outline focus-visible:z-10',
                iconColor
              )}
              part="prev-year-button"
              @click=${() => this.setYear(-1)}
              @keydown=${(ev: KeyboardEvent) => this.handleHeaderKeyDown(ev, 'year', -1, 'first')}
            ...
            <button
              type="button"
              tabindex="0"
              class=${cx('nav prev relative w-6 h-6 hover:cursor-pointer focus-visible:focus-outline focus-visible:z-10', iconColor)}
              part="prev-month-button"
              @click=${() => this.setMonth(-1)}
              @keydown=${(ev: KeyboardEvent) => this.handleHeaderKeyDown(ev, 'month', -1, 'middle')}
          ...
          <div
            tabindex="-1"
            class="month-label flex justify-center sd-headline sd-headline--size-base"
            part="month-label"
            aria-live="polite"
          >
            ${monthLabel}
          ...
          <div class="flex items-center">
            ...
            <button
              ...
              tabindex="0"
              class=${cx(
                'nav next relative w-6 h-6 hover:cursor-pointer focus-visible:focus-outline focus-visible:z-10',
                iconColor
              )}
            ...
            <button
              ...
              @click=${() => this.setYear(1)}
              @keydown=${(ev: KeyboardEvent) => this.handleHeaderKeyDown(ev, 'year', 1, 'last')}
                ...
                ${week.map((day, colIndex) => {
                  ...
                  return html`
                    <button
                      ...
                      aria-selected=${
                        isSelectedSingle || inSelectedRange || isRangeStart || isRangeEnd ? 'true' : 'false'
                      }
          ...
          >
            ...
            >
              <input
                ...
                class=${cx(
                  'min-w-0 grow focus:outline-none bg-transparent hover:cursor-pointer text-black',
